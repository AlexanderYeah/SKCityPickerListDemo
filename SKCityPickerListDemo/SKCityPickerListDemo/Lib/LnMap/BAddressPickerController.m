...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    if (tableView == _tableView) {
        return [[self.dictionary allKeys] count] + 1;
    }else{
        return 1;
    }
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (tableView == _tableView) {
        if (section > 0) {
            NSString *cityKey = [titleArray objectAtIndex:section - 1];
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == _tableView) {
        if ([cell isKindOfClass:[BCurrentCityCell class]]) {
            ...
            NSString *cityKey = [titleArray objectAtIndex:indexPath.section - 1];
            NSArray *array = [self.dictionary objectForKey:cityKey];
...
- (UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (tableView == _tableView) {
        ...
        if (section == 0){
            ...
            label.text = [titleArray objectAtIndex:section - 1];
        }
        return headerView;
    }else{
        return nil;
    ...
    if ([currentArray count] < 2 && ![currentArray containsObject:city]) {
        ...
        if (![currentArray containsObject:city]) {
            ...
            if (currentArray.count < 8) {
                ...
                // 数据已经超出8个，进行数据的更新,先把第一个删除，再添加一个
